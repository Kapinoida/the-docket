...
- **Fix:** Removed all 45 `console.log` statements from production code across 13 files. `console.error` (117 calls in catch/error handlers) and `console.warn` (7 calls for operational warnings) were retained — they serve legitimate error-handling and debugging purposes. Removed: `src/lib/caldav.ts` (18 `[Sync]`/`[Sync Fallback]` debug logs), `src/lib/recurrence.ts` (2 `[Recurrence]` instance logs), `src/lib/db.ts` (3 — pool config, tombstone, cleaner logs), `src/contexts/TaskEditContext.tsx` (3 "successfully" action logs), `src/hooks/usePeriodicSync.ts` (4 `[AutoSync]` progress logs — also simplified the success/fail branching), `src/components/SyncButton.tsx` (1 "Sync Results"), `src/components/CommandPalette.tsx` (1 "no page context"), `src/components/PwaRegister.tsx` (2 SW registration logs — converted failure to `console.warn`), `src/components/v2/SearchDialog.tsx` (2 "Selected tag/task"), `src/components/v2/Sidebar.tsx` (1 `onFolderSelect` stub), `src/pages/api/caldav/calendars.ts` (3 `[Discovery]` probe logs), `src/pages/api/caldav/repair.ts` (2 `[Repair]` task-count logs), `src/components/focus/FocusVisualizer.tsx` (1 dead commented-out debug line). In `caldav.ts`, the recurrence spawn block was simplified (removed the `if (spawnedId)` wrapper that only logged), and the tombstone count `if` block was removed (only contained a log). No functional changes — all `console.error`/`console.warn` error handlers and user-facing toast notifications remain intact.
...
- **Fixed in:** TBD (will reference commit on deploy)
- **Fix:**
  - **Sidebar container (primary):** Added `pb-[calc(52px+env(safe-area-inset-bottom,8px))] md:pb-0` to the sidebar container div in `LayoutWrapper.tsx:68`. On mobile, the sidebar's bottom edge now ends above the BottomTabBar (52px tab height + safe-area inset). On desktop (`md:`), `md:pb-0` resets the padding since the tab bar is hidden. The sidebar's `mt-auto` footer (Settings + Sync) naturally repositions above the padding.
  - **Calendar header compactness (secondary):** In `CalendarView.tsx:184-218`, four responsive tweaks to reduce wrapping on narrow phones: (1) "Tasks" button text hidden on mobile — `<span className="hidden md:inline">Tasks</span>`, icon only on small screens; (2) "Add Calendar" button text similarly hidden; (3) date label `min-w-[140px]` → `min-w-[100px] md:min-w-[140px]`; (4) "Today" button padding `px-4` → `px-3 md:px-4`. All text labels and full widths restore at `md:` breakpoint.
...
- **Fixed in:** Combined fix with BUG-011 — replaced hardcoded `380px` threshold with dynamic `popoverRef.current.offsetHeight` measurement via `ResizeObserver`. Desktop flip logic now uses actual popover height. Mobile (`< 768px`) renders as centered overlay with backdrop. See BUG-011 fix details.
...
- **Fixed in:** TBD (will reference commit on deploy)
- **Fix:**
  - **Root Cause 1 (historical):** Already fixed in June 2026 — `deleteCompletedTasks()` in `db.ts` now calls `deleteTaskReferences(taskId)` (line 398).
  - **Root Cause 2 (page deletion):** Replaced the raw `DELETE FROM tasks WHERE id IN (...)` in `src/pages/api/v2/pages.ts` (DELETE handler) with a SELECT of the orphaned task IDs followed by per-task `deleteTask(id)` calls. This ensures `createTombstone()` (CalDAV tombstone) and `deleteTaskReferences()` (v2Task node cleanup on other pages + `task_sync_meta` cleanup) run for every orphaned task, not just the bare row deletion.
  - **Root Cause 3 (null-taskId editor nodes):** Two-fold fix.
    1. **Editor race condition:** Removed the 500ms debounce from the v2Task creation effect in `src/components/v2/editor/extensions/TaskExtension.tsx`. Tasks are now created immediately when content is typed into a null-taskId node (the `isCreating.current` ref already prevents duplicate creations). Previously, page auto-save could persist a `taskId: null` node before the debounced `createTask()` POST had run, leaving a permanent ghost checkbox.
    2. **Save-time guard:** The PUT handler in `pages.ts` now strips v2Task nodes with `taskId: null` AND no text content from page content before persisting. These are definitively dead nodes (no backing task, no content) — defense in depth for any that slip through.
  - **Prevention (DB health check):** Added `src/migrations/005_clean_orphaned_v2task_nodes.sql` — a one-time, idempotent migration that recursively walks `pages.content` JSONB and strips (a) v2Task nodes with `taskId: null` and no text content, and (b) v2Task nodes whose numeric `taskId` no longer exists in the `tasks` table (catches orphans from any future leak path). Uses a temporary recursive PL/pgSQL function that is dropped after running. The 40 orphans found in the June 29 audit were already cleaned manually; this migration catches any remaining and prevents re-accumulation on redeploy.
...
- **Fix:** Applied combined approach (Options A + B + C) in `src/components/v2/DatePickerPopover.tsx`:
  - **Scrollable popover (A):** Added `max-h-[85vh] overflow-y-auto` to the popover container className. The popover never exceeds the viewport; if content is too tall (recurrence editor expanded), the popover scrolls internally and the Save/Clear buttons remain reachable.
  - **Mobile centered overlay (C):** On `window.innerWidth < 768`, the popover renders as a centered overlay (`top: 50%; left: 50%; transform: translate(-50%, -50%)`) with a semi-transparent backdrop (`bg-black/30`) that closes on click. This avoids the below/above flip logic entirely on small screens where it doesn't make sense.
  - **Dynamic height measurement (B/C):** Replaced the hardcoded `380px` flip threshold with `popoverRef.current.offsetHeight` measured via `ResizeObserver`. The observer fires when the recurrence editor expands/collapses, repositioning the popover dynamically. Also listens to `window resize` for viewport changes. Falls back to `400px` estimate if the ref isn't available yet.
  - No changes needed to `TaskEditor.tsx`, `TaskItem.tsx`, or `EditorTaskItem.tsx` — all consumers pass `triggerRef`/`onClose` which the new logic handles automatically. Also resolves BUG-009 (flip threshold too low) in the same pass.
