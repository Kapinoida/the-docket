...
export function SearchDialog() {
  ...
  const handleSelect = (item: SearchResult) => {
    ...
    if (item.type === 'page') {
      ...
      // For tasks, navigating to context is harder without knowing parent.
      // For now, let's assuming navigating to the dashboard or just a placeholder.
      // Ideally, the Search API should return parent_page_id for context.
      // Since we don't have that easily wired yet, let's just log or no-op?
      // Or maybe Task Edit Context?
      console.log('Selected task', item);
      // Future: Open task edit modal
    }
  };
            ...
            <Command.Group heading="Results" className="text-xs font-medium text-gray-500 mb-2">
                {results.map((item) => (
                    <Command.Item
                        ...
                        value={`${item.title} ${item.id}`} // Value used for internal filtering, but we drive via API
