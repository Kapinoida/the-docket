...
export default function VisualizationDropdown({ currentMode, onModeChange }: VisualizationDropdownProps) {
  ...
  const modes: { id: VisualizationMode; label: string; icon: React.ReactNode }[] = [
    ...
    { 
      id: 'constellation', 
      label: 'Net', // Renamed from Constellation as requested
  ...
  const currentOption = modes.find(m => m.id === currentMode);
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 bg-bg-secondary hover:bg-bg-tertiary border border-border-default rounded-lg text-sm text-text-primary transition-colors shadow-sm w-[160px] justify-between"
      >
      ...
      {isOpen && (
        <div className="absolute top-12 right-0 w-[200px] max-h-[300px] overflow-y-auto bg-bg-secondary border border-border-default rounded-lg shadow-xl z-50 py-1 styled-scrollbar">
          {modes.map((mode) => (
            <button
              key={mode.id}
              onClick={() => {
